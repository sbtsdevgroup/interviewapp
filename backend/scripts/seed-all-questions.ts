...
function seed() {
  ...
  const transaction = db.transaction((qs) => {
    for (const q of qs) {
      ...
      const duration = (q as any).duration_seconds !== undefined ? (q as any).duration_seconds : (defaultDurations[q.type] || 60);
      insert.run(uuidv4(), q.text, q.criteria, q.category, q.type, optionsJson, duration);
      insertedCount++;
    }
  });
